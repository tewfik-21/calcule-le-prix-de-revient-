                    ...
                    <table className="w-full text-left" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                      <thead className="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th className="px-6 py-4 text-sm font-semibold text-gray-600">Télephone</th>
                            <th className="px-6 py-4 text-sm font-semibold text-gray-600">Rôle</th>
                            <th className="px-6 py-4 text-sm font-semibold text-gray-600">Statut Premium</th>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {users.map(u => (
                          <tr key={u.id} className="hover:bg-gray-50">
                            <td className="px-6 py-4 font-medium text-gray-900">{u.full_name || 'Anonyme'}</td>
                              ...
                              {u.is_premium ? (
                                <div>
                                  <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs font-bold">
                                    {u.is_vip ? 'VIP' : 'Premium'}
                                  </span>
                                  {u.premium_until && (
                                    <div className="text-[10px] text-gray-500 mt-1">
                                      Jusqu'au {new Date(u.premium_until).toLocaleDateString()}
                                    </div>
                                  )}
                                </div>
                              ) : (
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {listings.map(l => (
                          <tr key={l.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 font-bold text-gray-900">{l.price}</td>
                            <td className="px-6 py-4">
                              <button 
                        ...
                        {payments.map(req => (
                          <tr key={req.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4">
                              ...
                              <div className="text-[10px] text-gray-500 mt-1">
                                {req.duration_months >= 12 ? 'Annuel (14 mois)' : 'Mensuel (1 mois)'}
                              </div>
                            </td>
                            <td className="px-6 py-4">
                                ...
                                <div className="flex gap-2">
                                  <button onClick={async () => {
                                    if(confirm('Accepter ce paiement ?')) {
                                      await approvePaymentRequest(req.id, req.user_id, req.requested_plan, req.duration_months || 1);
                                      loadData();
                                    }
