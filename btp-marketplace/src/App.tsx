...
import type { Listing, DealType, CategoryType, EquipmentType, Store, Tender, UserSession, AdBanner, JobOffer, Auction, Conversation } from './types';
...
export default function App() {
  ...
  useEffect(() => {
    if (banners.length > 1) {
      const interval = setInterval(() => {
        setSidebarBannerIndex((prev) => (prev + 1) % banners.length);
      }, 5000);
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session?.user) {
        ...
        const isVipActive = profile?.is_vip && (!profile?.premium_until || new Date(profile.premium_until) > new Date());
    ...
    const loadAdditionalData = async () => {
      const [fetchedStores, fetchedTenders, fetchedJobs, fetchedAuctions] = await Promise.all([
        fetchStores(),
        fetchTenders(),
        fetchJobOffers(),
        fetchAuctions()
  ...
  const [messageContext, setMessageContext] = useState<{ recipientId: string; listingId?: string; listingTitle?: string } | null>(null);
  ...
  const handleContactSeller = async () => {
    ...
    if (selectedListing) {
      if (!selectedListing.sellerId) {
        alert(lang === 'ar' ? "لا يمكن ترك رسالة، معرف الناشر غير متوفر." : "Impossible d'envoyer un message, l'ID du vendeur est manquant.");
        return;
      }
      setMessageContext({
        recipientId: selectedListing.sellerId,
  ...
  const handleContactJobAuthor = async (jobId: string, authorId: string) => {
    if (!user || user.id === 'guest') {
      setShowAuthModal(true);
      return;
    }
  ...
  // Handling Post Ad click
    ...
    if (!user.isPremium && !user.isVip && !user.isAdmin) {
    ...
    const maxStores = user.isAdmin ? 9999 : user.isVip ? 3 : 1;
  ...
  const compressImage = async (file: File): Promise<File> => {
    // If it's not an image (by type or extension), skip compression
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          // Add Watermark
          const watermarkText = "binadz.com";
          ctx.save();
          // Make it larger and more visible for testing
          const fontSize = Math.max(36, Math.floor(width / 15));
          ctx.font = `black ${fontSize}px sans-serif`;
          ctx.fillStyle = "rgba(255, 255, 255, 1)"; // 100% opaque white
          ctx.textAlign = "right";
  ...
  const handlePostJob = async (newJob: JobOffer, cvFile?: File) => {
    try {
      ...
      if (data && data[0]) {
        newJob.id = data[0].id;
        newJob.authorId = data[0].author_id; // Set authorId from database response
        if (finalCvUrl) newJob.cvUrl = finalCvUrl;
      } else {
        newJob.authorId = user?.id || "guest"; // Fallback to current user's ID
      }
  ...
  const handleUpdateStore = async (updatedStore: Store, logoFile?: File, bannerFile?: File) => {
    try {
      let logoUrl = updatedStore.logoUrl;
      let bannerUrl = updatedStore.bannerUrl;

      if (user) {
        if (logoFile) {
          const { error: err1 } = await supabase.storage.from('stores').upload(`${user.id}/logo_${logoFile.name}`, await compressImage(logoFile), { upsert: true });
          if (!err1) {
            const { data } = supabase.storage.from('stores').getPublicUrl(`${user.id}/logo_${logoFile.name}`);
            logoUrl = data.publicUrl;
          }
        }
        if (bannerFile) {
          const { error: err2 } = await supabase.storage.from('stores').upload(`${user.id}/banner_${bannerFile.name}`, await compressImage(bannerFile), { upsert: true });
          if (!err2) {
            const { data } = supabase.storage.from('stores').getPublicUrl(`${user.id}/banner_${bannerFile.name}`);
      ...
      updatedStore.logoUrl = logoUrl;
      updatedStore.bannerUrl = bannerUrl;
      ...
      alert('Erreur lors de la modification de la vitrine.');
    }
  };
    ...
    >
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-2 md:px-8 py-4 lg:py-6 grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 relative z-20 h-[calc(100vh-140px)] lg:h-[calc(100vh-80px)] overflow-y-auto overflow-x-hidden pb-24 lg:pb-0">
            ...
            {user && (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span>{lang === 'ar' ? 'صندوق الرسائل' : 'Boîte de réception'}</span>
                </div>
                {unreadMessages > 0 && (
        ...
        <div className={`col-span-1 flex flex-col h-full min-h-0 ${mobileView === 'map' ? 'hidden' : 'lg:col-span-6'}`}>
          ...
          {activeView === 'store_detail' && selectedStore && (
              <div className={`flex-1 flex flex-col ${mobileView === 'list' ? 'block' : 'hidden lg:flex'}`}>
                <div className="flex items-center justify-between mb-4">
                  <button 
                    onClick={() => setActiveView('stores')}
                    className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-1 hover:text-orange-500 transition w-fit"
                  >
                    ← Retour aux vitrines
                  </button>
                  {(user?.isAdmin || user?.id === selectedStore?.owner_id) && (
                    <button 
                      onClick={() => setShowEditStoreModal(true)}
                      className="bg-orange-500/10 text-orange-500 hover:bg-orange-500 hover:text-white transition px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center gap-2"
                    >
                      Modifier ma Vitrine
                    </button>
                  )}
                </div><div className="glass-card rounded-3xl border border-orange-500/20 overflow-hidden shrink-0 mb-6 relative">
                <button 
            ...
            <div className={`flex-1 flex flex-col ${mobileView === 'list' ? 'block' : 'hidden lg:flex'} overflow-y-auto pr-2 scrollbar-thin pb-20`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                ...
                <div className="glass-card rounded-3xl border border-orange-500 p-6 flex flex-col relative transform lg:-translate-y-4 shadow-2xl shadow-orange-500/10">
                  ...
                  <p className="text-[10px] text-orange-500/60 mb-4">Ou 25 000 DA / 12 Mois (2 mois gratuits)</p>
                  <div className="mb-6">
                    <span className="text-4xl font-black text-white">2 500</span>
                    <span className="text-[10px] font-black text-orange-500/60 uppercase ml-1">DA / Mois</span>
                  </div>
                  <ul className="space-y-3 flex-1 mb-6">
                    ...
                    <li className="flex items-center gap-2 text-[11px] font-medium text-slate-300"><Check className="h-4 w-4 text-orange-500 shrink-0"/> Vitrine Officielle</li>
                    <li className="flex items-center gap-2 text-[11px] font-medium text-slate-300"><Check className="h-4 w-4 text-orange-500 shrink-0"/> Assistance prioritaire</li>
                  </ul>
                ...
                {/* Pack VIP */}
                <div className="glass-card rounded-3xl border border-purple-500/50 p-6 flex flex-col hover:border-purple-500 transition duration-300 relative shadow-xl shadow-purple-500/5">
                  <h4 className="text-lg font-black text-purple-400 mb-1 uppercase tracking-widest">VIP</h4>
                  <p className="text-[10px] text-purple-500/60 mb-4">Ou 50 000 DA / 12 Mois (2 mois gratuits)</p>
                  <div className="mb-6">
                    <span className="text-4xl font-black text-white">5 000</span>
                    <span className="text-[10px] font-black text-purple-500/60 uppercase ml-1">DA / Mois</span>
                  </div>
                  <ul className="space-y-3 flex-1 mb-6">
                    <li className="flex items-center gap-2 text-[11px] font-medium text-slate-300"><Check className="h-4 w-4 text-purple-500 shrink-0"/> Tout du Pack Premium</li>
                    <li className="flex items-center gap-2 text-[11px] font-medium text-slate-300"><Check className="h-4 w-4 text-purple-500 shrink-0"/> Annonces toujours en Vedette</li>
                    <li className="flex items-center gap-2 text-[11px] font-medium text-slate-300"><Check className="h-4 w-4 text-purple-500 shrink-0"/> Contact direct avec acheteurs</li>
                    <li className="flex items-center gap-2 text-[11px] font-medium text-slate-300"><Check className="h-4 w-4 text-purple-500 shrink-0"/> Statistiques et tableau de bord</li>
                  </ul>
                  <button className="w-full bg-slate-800 hover:bg-slate-700 text-purple-400 hover:text-purple-300 font-black text-xs py-3 rounded-xl transition uppercase tracking-widest border border-purple-500/20">Sélectionner</button>
                </div>

                {/* Pack Entreprise */}
                <div className="glass-card rounded-3xl border border-emerald-500/50 p-6 flex flex-col hover:border-emerald-500 transition duration-300 relative">
                  <h4 className="text-lg font-black text-emerald-400 mb-1 uppercase tracking-widest">Entreprise</h4>
            ...
            <div className={`flex-1 flex flex-col ${mobileView === 'list' ? 'block' : 'hidden lg:flex'} overflow-y-auto pr-2 scrollbar-thin pb-20`}>
              <div className="flex items-center justify-between mb-6 border-b border-white/5 pb-4">
                <div className="flex items-center gap-3">
                  <button 
                    onClick={() => setActiveView('feed')} 
                    className="h-10 w-10 shrink-0 rounded-xl bg-slate-800 hover:bg-slate-700 flex items-center justify-center text-slate-300 transition"
                  >
                    <ArrowLeft className="h-5 w-5" />
                  </button>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div className="glass-card rounded-2xl border border-white/5 p-4 flex flex-col hover:border-white/20 transition">
                  ...
                  <div className="flex items-end gap-2">
                    <span className="text-2xl font-black text-white">
                      {storeInventory.reduce((acc, item) => acc + (item.auditDetails?.views || 0), 0)}
                    </span>
                  </div>
                </div>
                ...
                <div className="glass-card rounded-2xl border border-white/5 p-4 flex flex-col hover:border-white/20 transition">
                  ...
                  <div className="flex items-end gap-2">
                    <span className="text-2xl font-black text-white">
                      {storeInventory.reduce((acc, item) => acc + (item.auditDetails?.clicks || 0), 0)}
                    </span>
                  </div>
                </div>
                ...
                <div className="glass-card rounded-2xl border border-white/5 p-4 flex flex-col hover:border-white/20 transition">
                  ...
                  <div className="flex items-end gap-2">
                    <span className="text-2xl font-black text-white">
                      {(() => {
                        const views = storeInventory.reduce((acc, item) => acc + (item.auditDetails?.views || 0), 0);
                        const leads = storeInventory.reduce((acc, item) => acc + (item.auditDetails?.clicks || 0), 0);
                        return views > 0 ? ((leads / views) * 100).toFixed(1) : "0.0";
                      })()}%
                    </span>
                  </div>
                </div>
          ...
          {/* Jobs View */}
          {activeView === 'jobs' && (
            <JobsView mobileView={mobileView} jobs={jobs} onAddJobClick={handleAddJobClick} onContact={handleContactJobAuthor} />
          )}

          {/* Chat View */}
          {activeView === 'chat' && user && user.id !== 'guest' && (
            <ChatView user={user} t={t} lang={lang as string} initialConversation={activeConversation} />
          )}
        </div>
        ...
        <div className={`col-span-1 space-y-6 flex flex-col h-full ${mobileView === 'map' ? 'block lg:col-span-9' : 'hidden lg:flex lg:col-span-3'}`}>
          ...
          {/* Advertising Space (Multiple Ads) */}
          {banners && banners.length > 0 && (
            <div className="hidden lg:flex flex-col gap-4 shrink-0">
              {banners.slice(0, 3).map((banner, idx) => (
                <div key={banner.id || idx} className="relative rounded-2xl overflow-hidden border border-white/10 group cursor-pointer h-40">
                  <a href={banner.linkUrl} target="_blank" rel="noopener noreferrer">
                    ...
                    <div className="absolute bottom-3 left-3 right-3">
                      <h4 className="text-white font-black text-xs drop-shadow-md uppercase tracking-wider">{banner.sponsorName}</h4>
                    </div>
                  </a>
                </div>
              ))}
      ...
      {messageContext && user && (
        <MessageModal
          ...
          listingId={messageContext.listingId}
          listingTitle={messageContext.listingTitle}
          lang={lang as string}
        />
      )}
      ...
      {showEditStoreModal && selectedStore && (
        <EditStoreModal initialStore={selectedStore} user={user} onClose={() => setShowEditStoreModal(false)} onSubmit={handleUpdateStore} t={t} lang={lang as any} />
      )}
    </div>
  );
}
